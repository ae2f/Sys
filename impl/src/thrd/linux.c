/**
 * @file	linux.c
 * @brief	gnu_source clone2 (with redzone, page allocation)
 * */

#ifndef	ae2fsys_thrd_linux_h
#define	ae2fsys_thrd_linux_h

#include <ae2f/Sys/.thrd/core.h>

#if	AE2FSYS_THRD_LINUX
/**********************************/
...
ae2f_unused static ae2fsys_thrdres_t _linux_ae2fsys_thrd_runner(ae2fsys_thrdprm_t prm_stck) {
#define	prm_stck	ae2f_reinterpret_cast(ae2fsys_thrd* ae2f_restrict, prm_stck)
	...
	ae2f_expected_if(prm_stck && prm_stck->m_state__linux) {
		...
		ae2f_unused enum AE2FSYS_FTX_ ENUM;
...
{
	enum AE2FSYS_FTX_	L$$res = AE2FSYS_FTX_GOOD;
...
ae2f_MAC() ae2fsys_yield_thrd_imp(int ae2f_unused L)
{
